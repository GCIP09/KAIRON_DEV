...
interface Transaccion {
  ...
  monto: number; puntosGanados: number; comentarios?: string; createdAt: string;
  cancelada?: boolean;
  items?: any;
}
      ...
      <div className="glass-panel p-6 rounded-2xl w-full max-w-lg shadow-2xl max-h-[92vh] overflow-y-auto space-y-5"
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {form.categoria === 'ABARROTES' && (
            <div className="border-t pt-4 space-y-1.5" style={{ borderColor: 'var(--k-border)' }}>
              <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#818cf8' }}>Detalles de Abarrotes</p>
              <label className="text-xs k-muted font-semibold block">Marca/Distribuidor</label>
...
export default function App() {
  ...
  const [newSale, setNewSale] = useState({ clienteId: '', tipoNegocio: 'ROPA' as Transaccion['tipoNegocio'], monto: '', comentarios: '', puntosCanjeados: 0 });
  ...
  const [catalogFilter, setCatalogFilter] = useState<'TODOS' | 'ROPA' | 'PAPELERIA' | 'ABARROTES' | 'SERVICIO'>('TODOS');
  ...
  const handleCreateClient = async (e: React.FormEvent) => {
    ...
    if (backendStatus === 'online') {
      try {
        ...
        if (r.ok) { triggerAlert('success', 'Cliente registrado.'); loadData(); }
        else { const err = await r.json(); triggerAlert('error', err.error || 'Error.'); }
      } catch { triggerAlert('error', 'Error de red.'); }
    } else {
      setClientes(prev => [...prev, { id: prev.length + 1, nombre: newClient.nombre, telefono: newClient.telefono, saldoPuntos: 0 }]);
  ...
  const handleAddToCart = () => {
    ...
    if (selectedProdId.startsWith('manual-')) {
      if (!customPrice) return;
      let category: Producto['categoria'] = 'SERVICIO';
      let factor = 0;
      if (selectedProdId === 'manual-paca') {
        category = 'ROPA';
        factor = config.factorPuntosRopa;
      } else if (selectedProdId === 'manual-papeleria') {
        category = 'PAPELERIA';
        factor = config.factorPuntosPapeleria;
      } else if (selectedProdId === 'manual-abarrotes') {
        category = 'ABARROTES';
        factor = config.factorPuntosAbarrotes;
      }
      const name = customName.trim() || `Cobro Directo ${category === 'ROPA' ? 'Ropa' : category === 'PAPELERIA' ? 'Papelería' : category === 'ABARROTES' ? 'Abarrotes' : 'Servicios/Otros'}`;
    ...
    const [cat, idStr] = selectedProdId.split('-');
    const prod = productos.find(p => p.id === parseInt(idStr) && p.categoria === cat);
    if (!prod) return;

    const existing = cart.find(item => item.producto.id === prod.id && item.producto.categoria === prod.categoria);
    if (existing) {
      if (prod.stock !== 9999 && existing.cantidad >= prod.stock) {
        triggerAlert('error', `Stock insuficiente. Solo quedan ${prod.stock} unidades.`);
        return;
      }
      setCart(prev => prev.map(item => (item.producto.id === prod.id && item.producto.categoria === prod.categoria) ? { ...item, cantidad: item.cantidad + 1 } : item));
    } else {
      setCart(prev => [...prev, { producto: prod, cantidad: 1 }]);
    }
    setSelectedProdId('');
    setProdSearch('');
  };
  ...
  const handleCreateSale = async (e: React.FormEvent) => {
    ...
    const montoFinal    = Math.max(0, montoBase - puntosCanjeados * 0.5);
        ...
        if (clienteId && puntosCanjeados > 0) {
          await fetch(`${API}/api/transacciones`, {
            ...
            body: JSON.stringify({
              ...
              comentarios: `Canje: -${puntosCanjeados} pts = -$${(puntosCanjeados * 0.5).toFixed(2)} descuento`,
            }),
          });
        }
    ...
    setNewSale({ clienteId: '', tipoNegocio: 'ROPA', monto: '', comentarios: '', puntosCanjeados: 0 });
    setCart([]);
    setProdSearch('');
    setSelectedProdId('');
  };
  ...
  const puntosCanjeados     = newSale.puntosCanjeados || 0;
  const descuentoCanje      = puntosCanjeados * 0.5;
  const montoBase           = cart.reduce((acc, item) => acc + item.producto.precio * item.cantidad, 0);
  ...
  return (
    <div className="k-app" style={{ position: 'relative' }}>
      ...
      <aside className="k-sidebar w-64 flex flex-col justify-between sticky top-0 h-screen z-30">
        <div className="flex flex-col h-full">
          ...
          <div className="p-5 flex items-center gap-3" style={{ borderBottom: '1px solid var(--k-border)' }}>
            ...
            <div>
              <h1 className="kairon-logo-text text-lg leading-none">KAIRON</h1>
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-y-auto relative z-10">
        ...
        {/* Header */}
        <header className="k-header h-20 px-8 flex items-center justify-between sticky top-0 z-40">
          ...
          <div className="flex items-center gap-3">
            ...
            <button id="btn-nueva-venta" onClick={() => {
              setClientSearch('');
              setProdSearch('');
              setSelectedProdId('');
              setCart([]);
              setNewSale({ clienteId: '', tipoNegocio: 'ROPA', comentarios: '', puntosCanjeados: 0, monto: '' });
              setShowNewSaleModal(true);
            }}
              className="k-btn-primary flex items-center gap-2 px-4 text-sm"
              style={{ paddingTop: '0.625rem', paddingBottom: '0.625rem' }}>
              <Coins className="w-4 h-4" />Nueva Venta
            </button>
          </div>
        </header>
                ...
                <div className="glass-panel p-6 rounded-2xl lg:col-span-2 space-y-5">
                  ...
                  <table className="w-full k-table text-left border-collapse">
                    <thead><tr>
                      ...
                      <th className="text-right">Monto</th><th className="text-right">Puntos</th><th className="text-right">Hora</th>
                    </tr></thead>
                    <tbody>
                      {[...transacciones].reverse().slice(0, 6).map(t => (
                        <tr key={t.id} style={t.cancelada ? { opacity: 0.5 } : {}}>
                          ...
                          <td>
                            <p className="font-medium k-text flex items-center gap-2">
                ...
                <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between space-y-5">
                  <div>
                    ...
                    <div className="space-y-3">
                      {[
                        { icon: <Shirt className="w-5 h-5" />, label: 'Agregar Producto', sub: 'Ropa de paca o papelería', color: 'var(--k-green)', fn: () => setShowAddProductModal(true) },
                  ...
                  <div className="p-4 rounded-xl space-y-3"
                    ...
                    <ul className="text-xs space-y-1.5 k-muted">
                      ...
                      {config.habilitarAbarrotes && (
                        <li className="flex items-start gap-2">
                          <span className="w-1 h-1 rounded-full mt-1.5 shrink-0" style={{ background: '#818cf8' }} />
          ...
          {activeTab === 'inventario' && (
            <div className="space-y-5">
              ...
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <button id="btn-nuevo-producto" onClick={() => setShowAddProductModal(true)} className="k-btn-primary flex items-center gap-2 px-5 text-sm shrink-0" style={{ paddingTop: '0.75rem', paddingBottom: '0.75rem' }}>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
                {[
                  ...
                  { show: config.habilitarAbarrotes, label: 'Ventas Abarrotes', value: `$${ventasAbarrotes.toFixed(2)}`, color: '#818cf8' },
              ...
              <div className="glass-panel p-6 rounded-2xl">
                ...
                <table className="w-full k-table text-left border-collapse">
                  <thead><tr>
                    <th>ID</th><th>Tipo</th><th>Cliente</th><th>Notas</th>
                    <th className="text-right">Monto</th><th className="text-right">Puntos</th><th className="text-right">Fecha</th>
                  ...
                  <tbody>
                    {[...transacciones].reverse().map(t => (
                      <tr key={t.id} style={t.cancelada ? { opacity: 0.6 } : {}}>
                        ...
                        <td className="text-right text-xs k-muted">{new Date(t.createdAt).toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'short' })}</td>
                        <td className="text-right">
                          {t.cancelada ? (
                            <span className="text-[9px] bg-rose-500/10 text-rose-400 px-2 py-1 rounded font-extrabold uppercase tracking-wide">
                              Cancelada
                            </span>
                          ) : (
                            <button
                              ...
                              className="p-1 px-2 text-rose-400 hover:text-rose-300 rounded-lg transition-colors border hover:bg-rose-500/10 text-xs font-bold"
                              style={{ background: 'var(--k-nav-hover)', borderColor: 'rgba(239,68,68,0.2)' }}
              ...
              <div className="glass-panel rounded-2xl overflow-hidden">
                <table className="w-full k-table text-left border-collapse">
                  <thead><tr>
                    ...
                    <th className="px-6 text-right">Puntos</th><th className="px-6 text-right">Equivalencia</th><th className="px-6 text-right">Acciones</th>
                  </tr></thead>
                  <tbody>
                    {filteredClientes.map(c => (
                      <tr key={c.id} className="group">
                        ...
                        <td className="px-6 text-right font-extrabold text-base" style={{ color: 'var(--k-gold)' }}>{c.saldoPuntos} pts</td>
                        <td className="px-6 text-right font-medium k-muted">${(c.saldoPuntos * 0.5).toFixed(2)} MXN</td>
                        <td className="px-6 text-right">
                          <div className="flex items-center justify-end gap-2">
            ...
            <div className="space-y-5">
              <div className="glass-panel p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                ...
                <div className="flex gap-8">
                  {[
                    { label: 'Puntos', value: clienteDetalle.saldoPuntos, color: 'var(--k-gold)' },
                    { label: 'Equiv. MXN', value: `$${(clienteDetalle.saldoPuntos*0.5).toFixed(2)}`, color: 'var(--k-text)' },
                    { label: 'Compras', value: transaccionesCliente(clienteDetalle.id).filter(t => !t.cancelada).length, color: 'var(--k-green)' },
                  ].map((s, i) => (
              ...
              <div className="glass-panel p-6 rounded-2xl">
                ...
                {transaccionesCliente(clienteDetalle.id).length === 0 ? (
                  ...
                  <table className="w-full k-table text-left border-collapse">
                    <thead><tr><th>ID</th><th>Tipo</th><th>Notas</th><th className="text-right">Monto</th><th className="text-right">Puntos</th><th className="text-right">Fecha</th></tr></thead>
                    <tbody>
                      {transaccionesCliente(clienteDetalle.id).map(t => (
                        <tr key={t.id} style={t.cancelada ? { opacity: 0.6 } : {}}>
                          <td className="font-mono k-muted text-xs">#{t.id}</td>
                          <td><CategoriaBadge cat={t.tipoNegocio} /></td>
                          <td className="k-muted text-xs">
                            ...
                            {t.comentarios || '—'}</td>
                          <td className="text-right font-bold k-text">${Number(t.monto).toFixed(2)}</td>
          ...
          {activeTab === 'reportes' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {[
                  { label: 'Ticket Promedio', value: `$${activeTransacciones.length > 0 ? (totalCaja/activeTransacciones.length).toFixed(2) : '0.00'}`, icon: <TrendingUp className="w-5 h-5" />, color: 'var(--k-gold)' },
                  { label: 'Puntos Otorgados', value: `${totalPuntos} pts`, icon: <Sparkles className="w-5 h-5" />, color: 'var(--k-green)' },
                  { label: 'Clientes con Puntos', value: `${clientes.filter(c=>c.saldoPuntos>0).length} / ${clientes.length}`, icon: <UserCheck className="w-5 h-5" />, color: 'var(--k-teal)' },
                ].map((k, i) => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-panel p-6 rounded-2xl space-y-5">
                  ...
                  <MiniBarChart data={[
                    ...
                    config.habilitarAbarrotes && { label: 'Abarrotes', value: ventasAbarrotes, color: 'linear-gradient(135deg,#818cf8,color-mix(in srgb,#818cf8 70%,#fff))' },
                  ...
                  <div className="space-y-3 pt-2">
                    {[
                      ...
                      { show: config.habilitarAbarrotes, label: 'Abarrotes', value: ventasAbarrotes, pct: totalCaja > 0 ? (ventasAbarrotes/totalCaja*100).toFixed(1) : '0', color: '#818cf8' },
              ...
              <form onSubmit={handleSaveConfig} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="glass-panel p-6 rounded-2xl flex flex-col justify-between space-y-5">
                  <div className="space-y-5">
                    ...
                    <div className="space-y-4">
                      <Field label={`Porcentaje Ropa (${(configForm.factorPuntosRopa * 100).toFixed(0)}%)`}>
                        <div className="flex items-center gap-3">
                          <input
                            type="range"
                            min="0"
                            max="0.30"
                            step="0.01"
                            value={configForm.factorPuntosRopa}
                            onChange={e => setConfigForm({ ...configForm, factorPuntosRopa: parseFloat(e.target.value) })}
                          ...
                          <input
                            ...
                            step="0.01"
                            value={configForm.factorPuntosRopa}
                            onChange={e => setConfigForm({ ...configForm, factorPuntosRopa: parseFloat(e.target.value) || 0 })}
                            className="k-input w-24 text-center font-bold"
                          />
                        </div>
                      </Field>

                      <Field label={`Porcentaje Papelería (${(configForm.factorPuntosPapeleria * 100).toFixed(0)}%)`}>
                        <div className="flex items-center gap-3">
                          <input
                            type="range"
                            min="0"
                            max="0.30"
                            step="0.01"
                            value={configForm.factorPuntosPapeleria}
                            onChange={e => setConfigForm({ ...configForm, factorPuntosPapeleria: parseFloat(e.target.value) })}
                          ...
                          <input
                            ...
                            step="0.01"
                            value={configForm.factorPuntosPapeleria}
                            onChange={e => setConfigForm({ ...configForm, factorPuntosPapeleria: parseFloat(e.target.value) || 0 })}
                            className="k-input w-24 text-center font-bold"
                          />
                        </div>
                      </Field>

                      <Field label={`Porcentaje Abarrotes (${(configForm.factorPuntosAbarrotes * 100).toFixed(0)}%)`}>
                        <div className="flex items-center gap-3">
                          <input
                            type="range"
                            min="0"
                            max="0.30"
                            step="0.01"
                            value={configForm.factorPuntosAbarrotes}
                            onChange={e => setConfigForm({ ...configForm, factorPuntosAbarrotes: parseFloat(e.target.value) })}
                          ...
                          <input
                            ...
                            step="0.01"
                            value={configForm.factorPuntosAbarrotes}
                            onChange={e => setConfigForm({ ...configForm, factorPuntosAbarrotes: parseFloat(e.target.value) || 0 })}
                            className="k-input w-24 text-center font-bold"
                          />
                        </div>
                      </Field>
                  ...
                  <div className="pt-5 border-t" style={{ borderColor: 'var(--k-border)' }}>
      ...
      {showNewSaleModal && (
        <Modal title="Registrar Venta / Transacción" icon={<Coins className="w-5 h-5 k-gold" />} onClose={() => setShowNewSaleModal(false)} maxWidth="max-w-6xl">
          <form onSubmit={handleCreateSale} className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            
            {/* Columna Izquierda: Catálogo de Productos y Cobros Directos */}
            <div className="lg:col-span-7 space-y-4">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                {/* Buscador local */}
                <div className="relative w-full md:w-64">
                  <Search className="absolute left-3 top-3 w-4 h-4 k-muted" />
                  <input
                    type="text"
                    placeholder="Buscar producto/código..."
                    value={prodSearch}
                    onChange={e => setProdSearch(e.target.value)}
                    className="k-input pl-10 py-2 text-xs"
                  />
                  {prodSearch && (
                    <button
                      type="button"
                      onClick={() => setProdSearch('')}
                      className="absolute right-3 top-2.5 text-xs text-rose-400 hover:text-rose-300"
                    >
                      Limpiar
                    </button>
                  )}
                ...
                <div className="flex gap-1.5 overflow-x-auto pb-1 max-w-full">
                  {([
                    ...
                    { value: 'ABARROTES', label: 'Abarrotes', show: config.habilitarAbarrotes },
                    { value: 'SERVICIO', label: 'Servicios', show: config.habilitarServicios }
                  ] as { value: typeof catalogFilter; label: string; show: boolean }[])
                    .filter(f => f.show)
                    .map(f => (
                      <button
                        key={f.value}
                        ...
                        style={{
                          background: catalogFilter === f.value ? 'var(--k-gold)' : undefined,
                          color: catalogFilter === f.value ? '#000' : undefined
                        }}
                      >
                        {f.label}
                      </button>
              ...
              {/* Botones de Cobro Directo (Manual) */}
              <div className="rounded-xl p-3 border space-y-2" style={{ background: 'var(--k-panel-bg)', borderColor: 'var(--k-border)' }}>
                <p className="text-[10px] uppercase tracking-wider font-extrabold k-muted">Cobro Directo / Concepto Manual</p>
                <div className="flex flex-wrap gap-2">
                  {[
                    { show: config.habilitarRopa, id: 'manual-paca', label: 'Ropa Paca', color: '#c084fc' },
                    { show: config.habilitarPapeleria, id: 'manual-papeleria', label: 'Papelería', color: '#4ade80' },
                    { show: config.habilitarAbarrotes, id: 'manual-abarrotes', label: 'Abarrotes', color: '#818cf8' },
                    { show: config.habilitarServicios, id: 'manual-servicio', label: 'Servicio/Otro', color: '#60a5fa' }
                  ]
                    .filter(opt => opt.show)
                    .map(opt => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setSelectedProdId(opt.id);
                          setCustomName('');
                          setCustomPrice('');
                        }}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${
                          selectedProdId === opt.id
                            ? 'bg-slate-700 text-white border-transparent'
                            : 'hover:brightness-110'
                        }`}
                        style={{
                          background: selectedProdId === opt.id ? opt.color : `color-mix(in srgb, ${opt.color} 12%, transparent)`,
                          borderColor: selectedProdId === opt.id ? 'transparent' : `color-mix(in srgb, ${opt.color} 25%, transparent)`,
                          color: selectedProdId === opt.id ? '#000' : opt.color
                        }}
                      >
                ...
                {/* Formulario de Cobro Directo si está seleccionado */}
                {selectedProdId.startsWith('manual-') && (
                  <div className="rounded-lg p-3 space-y-3 mt-2 border animate-in fade-in slide-in-from-top-2 duration-200"
                    style={{ background: 'color-mix(in srgb, var(--k-gold) 6%, var(--k-surface))', borderColor: 'var(--k-border)' }}>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--k-gold)' }}>
                        Concepto Manual Activo ({
                          selectedProdId === 'manual-paca' ? `Ropa ${(config.factorPuntosRopa*100).toFixed(0)}% pts` :
                          selectedProdId === 'manual-papeleria' ? `Papelería ${(config.factorPuntosPapeleria*100).toFixed(0)}% pts` :
                          selectedProdId === 'manual-abarrotes' ? `Abarrotes ${(config.factorPuntosAbarrotes*100).toFixed(0)}% pts` :
                          'Servicio/Otros 0% pts'
                        })
                      </span>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-[9px] k-muted uppercase font-bold">Concepto (Opcional)</label>
                        <input
                          type="text"
                          placeholder="Ej. Prenda sin registrar, Copias"
                          value={customName}
                          onChange={e => setCustomName(e.target.value)}
                          className="k-input py-1 px-2 text-xs"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] k-muted uppercase font-bold">Precio del Cobro ($)</label>
                        <input
                          type="number"
                          step="0.01"
                          placeholder="0.00"
                    ...
                    <button
                      ...
                      className="k-btn-primary py-1.5 text-xs font-bold w-full disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div className="rounded-xl border p-4 space-y-3" style={{ background: 'var(--k-panel-bg)', borderColor: 'var(--k-border)' }}>
                <p className="text-[10px] uppercase tracking-wider font-extrabold k-muted">Inventario Disponible</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[26rem] overflow-y-auto pr-1">
                  {productos
                    .filter(p => {
                      ...
                      const matchesCategory = catalogFilter === 'TODOS' || p.categoria === catalogFilter;
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            const existing = cart.find(item => item.producto.id === p.id && item.producto.categoria === p.categoria);
                            if (existing) {
                              if (p.stock !== 9999 && existing.cantidad >= p.stock) {
                          ...
                          className={`flex items-center justify-between p-3 rounded-xl border text-left transition-all ${
                            isOutOfStock
                              ? 'opacity-40 cursor-not-allowed border-dashed bg-slate-900/10'
                              : qtyInCart > 0
                              ? 'bg-slate-800/60 shadow-lg border-[var(--k-gold)]'
                              : 'bg-slate-800/20 hover:bg-slate-800/40 hover:border-slate-600'
                          }`}
                          style={{
                            borderColor: qtyInCart > 0 ? 'var(--k-gold)' : 'var(--k-border)',
                            background: qtyInCart > 0 ? 'color-mix(in srgb, var(--k-gold) 5%, var(--k-panel-bg))' : undefined
                          }}
                        >
                          <div className="min-w-0 flex-1">
                            <p className="font-bold text-xs k-text truncate flex items-center gap-1.5">
                              {p.nombre}
                              {qtyInCart > 0 && (
                                <span className="bg-emerald-500/20 text-emerald-400 text-[10px] font-extrabold px-1.5 py-0.5 rounded-full border border-emerald-500/30">
                                  {qtyInCart} pzs
                                </span>
                              )}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-[9px] uppercase font-extrabold px-1 rounded" style={{
                                background: p.categoria === 'ROPA' ? 'rgba(168,85,247,0.1)' : p.categoria === 'PAPELERIA' ? 'rgba(34,197,94,0.1)' : p.categoria === 'ABARROTES' ? 'rgba(129,140,248,0.1)' : 'rgba(59,130,246,0.1)',
                                color: p.categoria === 'ROPA' ? '#c084fc' : p.categoria === 'PAPELERIA' ? '#4ade80' : p.categoria === 'ABARROTES' ? '#818cf8' : '#60a5fa'
                              }}>{p.categoria}</span>
                              <span className="text-[10px] k-muted truncate font-medium">{p.codigoBarras || 'Sin barra'}</span>
                            </div>
                          </div>
                  ...
                  {productos.filter(p => {
                    ...
                    <div className="col-span-2 text-center py-12 text-xs k-muted">No se encontraron productos en esta categoría</div>
                  )}
                </div>
              </div>
            </div>

            {/* Columna Derecha: Detalle de Venta, Carrito y Cobro */}
            <div className="lg:col-span-5 space-y-4 border-t lg:border-t-0 lg:border-l pt-6 lg:pt-0 lg:pl-6" style={{ borderColor: 'var(--k-border)' }}>
              
              {/* Selector de Cliente Searchable Combobox */}
              <Field label="Vincular Cliente (Opcional)">
                <div className="relative" ref={clientSelectRef}>
                  ...
                  <button
                    ...
                    className="k-input w-full text-left flex justify-between items-center py-2 px-3 text-xs"
                  ...
                  {showClientDropdown && (
                    <div className="absolute top-full left-0 right-0 mt-1.5 max-h-64 overflow-y-auto glass-panel p-2 shadow-2xl z-[10000] rounded-xl border text-[var(--k-text)] space-y-2"
                      ...
                      <div className="space-y-0.5 max-h-44 overflow-y-auto pr-0.5">
                        ...
                        {clientes
                          .filter(c => {
                            const matchesSearch = c.nombre.toLowerCase().includes(clientSearch.toLowerCase()) || 
                        ...
                        {clientes.filter(c => 
                          ...
                          <div className="text-center py-4 text-xs k-muted">No se encontraron clientes</div>
                        )}
                      </div>
                    </div>
                  )}
              ...
              {/* Carrito de Compras */}
              {cart.length > 0 ? (
                <div className="rounded-xl p-4 space-y-3" style={{ background: 'var(--k-panel-bg)', border: '1px solid var(--k-border)' }}>
                  <span className="text-[10px] font-extrabold uppercase tracking-wider k-muted">Artículos en esta Venta</span>
                        ...
                        <div key={`${p.categoria}-${p.id}`} className="flex items-center justify-between gap-3 text-sm border-b pb-2.5 last:border-0 last:pb-0" style={{ borderColor: 'var(--k-border)' }}>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[9px] uppercase font-extrabold px-1 py-0.2 rounded" style={{
                                background: p.categoria === 'ROPA' ? 'rgba(168,85,247,0.1)' : p.categoria === 'PAPELERIA' ? 'rgba(34,197,94,0.1)' : p.categoria === 'ABARROTES' ? 'rgba(129,140,248,0.1)' : 'rgba(59,130,246,0.1)',
                                color: p.categoria === 'ROPA' ? '#c084fc' : p.categoria === 'PAPELERIA' ? '#4ade80' : p.categoria === 'ABARROTES' ? '#818cf8' : '#60a5fa'
                              }}>{p.categoria}</span>
                          ...
                          <div className="flex items-center gap-1 shrink-0 bg-slate-800/40 border border-slate-700/50 rounded-lg p-0.5">
                            <button
                              type="button"
                              onClick={() => handleUpdateCartQty(p.id, p.categoria, item.cantidad - 1)}
                              className="w-5.5 h-5.5 rounded flex items-center justify-center bg-slate-800 text-slate-300 hover:bg-slate-700 font-extrabold text-xs"
                          ...
                          <span className="font-bold k-text shrink-0 min-w-[65px] text-right text-xs">${subtotal.toFixed(2)}</span>
                          <button
                            type="button"
                ...
                <div className="rounded-xl p-8 text-center k-muted text-xs border border-dashed flex flex-col items-center justify-center gap-2" style={{ borderColor: 'var(--k-border)' }}>
                  <Coins className="w-8 h-8 opacity-25" />
                  <span>El carrito está vacío. Haz clic en los productos a la izquierda.</span>
                </div>
              )}

              <Field label="Comentarios / Notas">
                <input type="text" placeholder="Ej. Playera Adidas + cuaderno" value={newSale.comentarios} onChange={e => setNewSale({...newSale, comentarios: e.target.value})} className="k-input text-xs" />
              </Field>

              <Field label="Monto Total del Carrito ($)">
                <input type="text" disabled value={`$${montoBase.toFixed(2)} MXN`} className="k-input bg-slate-900/30 opacity-70 cursor-not-allowed font-bold text-xs" />
              </Field>

              {/* ── SECCIÓN DE CANJE DE PUNTOS ─────────────────────────────── */}
              {clienteSeleccionado && puntosDisponibles > 0 && montoBase > 0 && (
                <div className="rounded-xl p-4 space-y-3 border" style={{ background: `color-mix(in srgb, var(--k-teal) 8%, var(--k-sidebar))`, borderColor: 'color-mix(in srgb, var(--k-teal) 25%, transparent)' }}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Coins className="w-4 h-4" style={{ color: 'var(--k-teal)' }} />
                      <span className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--k-teal)' }}>Canjear Puntos</span>
                    </div>
                  ...
                  <div className="space-y-2">
                    <input
                      type="range"
                      min={0}
                      max={Math.min(puntosDisponibles, Math.floor(montoBase / 0.5))}
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          max={Math.min(puntosDisponibles, Math.floor(montoBase / 0.5))}
                          value={puntosCanjeados}
                          onChange={e => setNewSale({...newSale, puntosCanjeados: Math.min(parseInt(e.target.value)||0, puntosDisponibles, Math.floor(montoBase/0.5))})}
                          className="k-input text-center w-20 py-1 px-2 text-xs"
                        />
                        <span className="k-muted">pts</span>
                      </div>
                      <span className="k-muted">{Math.min(puntosDisponibles, Math.floor(montoBase / 0.5))} pts máx</span>
                    </div>
                  </div>
                  {puntosCanjeados > 0 && (
                    <div className="rounded-lg p-2.5 text-xs space-y-1" style={{ background: 'color-mix(in srgb, var(--k-teal) 12%, transparent)' }}>
                      ...
                      <div className="flex justify-between" style={{ color: 'var(--k-teal)' }}>
                        <span>Descuento ({puntosCanjeados} pts × $0.50):</span>
                        <span className="font-bold font-mono">-${descuentoCanje.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-1" style={{ borderColor: 'var(--k-border)' }}>
                        <span className="font-bold k-text">Total a cobrar:</span>
                        <span className="font-extrabold text-sm" style={{ color: 'var(--k-green)' }}>${montoFinal.toFixed(2)}</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {puntosPreview > 0 && (
                <div className="rounded-xl p-3 text-xs flex items-center gap-2 relative overflow-hidden border"
                  style={{ background: `color-mix(in srgb, var(--k-gold) 10%, var(--k-sidebar))`, borderColor: 'var(--k-border)' }}>
                  <Sparkles className="w-4 h-4 k-gold shrink-0" />
                  <span className="k-muted">Se otorgarán</span>
                  <strong className="k-gold mx-0.5">{puntosPreview} puntos</strong>
                  <span className="k-muted">al cliente</span>
                  <TrinitySparkle show={false} />
                </div>
              )}

              <SubmitBtn>{puntosCanjeados > 0 ? `Registrar Cobro $${montoFinal.toFixed(2)}` : 'Registrar Cobro'}</SubmitBtn>
            </div>
